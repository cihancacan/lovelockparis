...
import { ArrowLeft, MapPin, Navigation, Lock, Loader2, Smartphone } from 'lucide-react';
  ...
  useEffect(() => {
    const initAR = async () => {
      try {
        ...
        setGpsLoading(false); // On débloque pour éviter l'écran noir si erreur
      }
    };
  ...
  return (
    <div className="fixed inset-0 bg-black overflow-hidden font-sans text-white">
      ...
      <div className="absolute top-4 left-4 z-50">
        <Button variant="outline" onClick={() => router.back()} className="bg-black/40 backdrop-blur border-white/20 text-white hover:bg-black/60">
      ...
      {(isAtBridge || debugMode) && (
        <>
          ...
          <div className="absolute bottom-10 left-0 right-0 text-center z-20 pointer-events-none">
            <div className="inline-block bg-black/40 backdrop-blur-md px-6 py-3 rounded-full text-white text-sm font-bold border border-white/20 animate-pulse">
