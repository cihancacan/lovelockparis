  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8 mt-6">
      <div className="bg-slate-800/80 backdrop-blur rounded-xl p-4 border border-slate-700/50 flex items-center justify-between shadow-lg">
        <div><div className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Live Buyers</div><div className="text-2xl font-black text-white flex items-center gap-2"><span className="relative flex h-3 w-3"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span><span className="relative inline-flex rounded-full h-3 w-3 bg-emerald-500"></span></span>{buyers}</div></div><UsersIcon className="text-emerald-500/20" size={32} />
      </div>
      <div className="bg-slate-800/80 backdrop-blur rounded-xl p-4 border border-slate-700/50 flex items-center justify-between shadow-lg">
        <div><div className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">24h Volume</div><div className="text-2xl font-black text-emerald-400">$380,000</div></div><BarChart3 className="text-emerald-500/20" size={32} />
      </div>
      <div className="bg-slate-800/80 backdrop-blur rounded-xl p-4 border border-slate-700/50 flex items-center justify-between shadow-lg">
        <div><div className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Floor Price</div><div className="text-2xl font-black text-white">$29.99</div></div><TrendingUp className="text-blue-500/20" size={32} />
      </div>
      <div className="bg-slate-800/80 backdrop-blur rounded-xl p-4 border border-slate-700/50 flex items-center justify-between shadow-lg">
        <div><div className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Total Trades</div><div className="text-2xl font-black text-white">12.4K</div></div><Activity className="text-purple-500/20" size={32} />
      </div>
    </div>
  );
};
...
function MarketplaceContent() {
  ...
  useEffect(() => { setMounted(true); loadMarketplaceLocks(); }, []);

  const loadMarketplaceLocks = async () => {
    try {
      const { data, error } = await supabase.from('locks').select('*').or('status.eq.For_Sale,status.eq.Reserved_Admin').order('created_at', { ascending: false });
      ...
      const formattedLocks = (data || []).map(lock => {
        ...
        return {
          id: lock.id, zone: lock.zone || 'Standard', skin: lock.skin || 'Gold', price: Number(finalPrice),
          views_count: (lock.views_count || 0) + 100, boost_level: boostLvl, price_increase: ((lock.id % 20) * 5) + 10,
          viewers_now: viewers, is_golden: isGolden, content_text: ''
        };
      });
      const vips = formattedLocks.filter(l => l.boost_level === 'vip' || l.boost_level === 'golden' || l.price >= 500).slice(0, 10);
      setVipLocks(vips);
        ...
        <div className="container mx-auto relative z-10">
          ...
          <div className="flex flex-col md:flex-row gap-3 bg-slate-800/50 p-2 rounded-xl border border-slate-700 mt-8">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search Lock ID #..." className="pl-10 h-11 bg-slate-900 border-slate-700 text-white focus:ring-emerald-500 focus:border-emerald-500" value={search} onChange={(e) => setSearch(e.target.value)}/>
            </div>
                ...
                {vipLocks.map((lock) => (
                  <Card key={lock.id} onClick={() => handleQuickBuy(lock.id, lock.price)} className={`cursor-pointer border-2 shadow-xl overflow-hidden hover:scale-[1.02] transition-transform group ${lock.is_golden ? 'border-amber-400/50' : 'border-purple-200'}`}>
                    ...
                    <CardContent className="p-4 bg-white">
                      ...
                      <div className="h-24 bg-slate-50 rounded-lg mb-4 flex items-center justify-center relative overflow-hidden border border-slate-100 shadow-inner">
                      ...
                      <div className="flex justify-between items-end mb-2">
                         <div><p className="text-[10px] text-slate-400 font-bold uppercase">Price</p><div className="text-2xl font-bold text-slate-900">${lock.price.toLocaleString()}</div></div>
                         <div className="text-right text-xs text-emerald-600 font-bold bg-emerald-50 px-2 py-1 rounded"><TrendingUp className="h-3 w-3 inline mr-1" />+{lock.price_increase}%</div>
                      </div>
                      <Button size="sm" className={`w-full font-bold shadow-lg h-9 text-xs ${lock.is_golden ? 'bg-amber-600 hover:bg-amber-700' : 'bg-purple-600 hover:bg-purple-700'}`}>Buy Now</Button>
                    </CardContent>
                  </Card>
                ))}
              </div>
              ...
              return (
                <div key={lock.id} onClick={() => handleQuickBuy(lock.id, lock.price)} className={`relative group cursor-pointer rounded-xl border transition-all duration-200 hover:-translate-y-1 hover:shadow-lg overflow-hidden bg-white ${borderClass}`}>
                  ...
                  <div className="p-3 flex flex-col h-full items-center text-center">
                    <div className="relative w-16 h-16 mb-2"><Image src={getSkinImage(lock.skin)} alt={lock.skin} fill className="object-contain group-hover:scale-110 transition-transform duration-300 drop-shadow-sm" /></div>
