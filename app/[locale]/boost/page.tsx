...
const boostPackages: BoostPackage[] = [
  { id: 'basic', title: 'Basic Boost', price: 19.99, duration: '7 days', color: 'bg-blue-500', borderColor: 'border-blue-200', icon: Sparkles, badge: 'BOOSTED', features: ['Top 50 rotation', 'Basic badge', '2x Views'] },
  { id: 'premium', title: 'Premium Boost', price: 49.99, duration: '14 days', color: 'bg-amber-500', borderColor: 'border-amber-200', icon: Crown, badge: 'PREMIUM', features: ['Top 10 Fixed', 'Premium badge', '5x Views'] },
  { id: 'vip', title: 'VIP Featured', price: 99.99, duration: '30 days', color: 'bg-purple-600', borderColor: 'border-purple-200', icon: Trophy, badge: 'VIP', features: ['Homepage Feature', 'VIP Trophy', '10x Views'] }
];
...
function BoostPageContent() {
  ...
  useEffect(() => {
    // Si pas connecté, on charge quand même la page (mode vitrine)
    if (!user) {
      setLoading(false);
      return;
    }
    ...
    const loadData = async () => {
      try {
        const { data, error } = await supabase
          .from('locks')
          .select('*') // On prend tout (zone, skin...)
          .eq('owner_id', user.id)
        ...
        const paramId = searchParams.get('lock_id');
        if (paramId) {
          setSelectedLockId(parseInt(paramId));
        } else if (locks.length > 0) {
          setSelectedLockId(locks[0].id); // Sélectionne le 1er par défaut
        }
      } catch (e) {
        console.error(e);
      } finally {
  ...
  const handleBoostPurchase = async () => {
    if (!user) {
      // Sauvegarde intention et redirection
    ...
    // Redirection vers Checkout avec TOUTES les infos (Zone/Skin) pour éviter les bugs API
    const params = new URLSearchParams({
      type: 'boost',
      lock_id: selectedLockId.toString(),
      package: selectedPackage,
      price: pkg.price.toString(),
      // On passe les infos du lock pour que le Checkout ne plante pas
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-slate-50"><Loader2 className="animate-spin h-10 w-10 text-slate-400"/></div>;
            ...
            <div>
                ...
                {!user ? (
                    <div className="p-8 bg-white border-2 border-dashed border-slate-200 rounded-xl text-center">
                        ...
                        <p className="text-slate-500 mb-4 font-medium">Login to view your collection.</p>
          ...
          <div>
            <Card className="sticky top-24 shadow-xl border-amber-100">
              <CardContent className="p-6 space-y-4">
                ...
                {/* On montre le cadenas sélectionné */}
                {activeLock && (
                   <div className="text-xs text-slate-500 text-right">{activeLock.zone} • {activeLock.skin}</div>
                )}
