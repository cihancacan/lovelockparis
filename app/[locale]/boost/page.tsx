...
// --- PACKAGES ---
const boostPackages = [
  { id: 'basic', title: 'Quick Spark', price: 14.99, duration: '5 Days', views: '3x', color: 'from-blue-500 to-cyan-400', border: 'border-cyan-500', icon: Zap, features: ['Top 50 Rotation', 'Blue Badge'] },
  { id: 'premium', title: 'Market Heat', price: 29.99, duration: '10 Days', views: '10x', color: 'from-orange-500 to-red-500', border: 'border-orange-500', icon: Sparkles, popular: true, features: ['Top 20 Fixed', 'Fire Badge', 'Emailing'] },
  { id: 'vip', title: 'Diamond Domination', price: 59.99, duration: '15 Days', views: '25x', color: 'from-purple-600 to-pink-500', border: 'border-purple-500', icon: Trophy, features: ['Homepage Cover', 'Golden Border', 'Top 3 Guarantee'] }
];
...
function BoostPageContent() {
  ...
  // On initialise directement avec l'URL si possible
  const [selectedLockId, setSelectedLockId] = useState<number | null>(() => {
    const pid = searchParams.get('lock_id');
  ...
  useEffect(() => {
    if (user) {
        ...
        supabase.from('locks').select('*').eq('owner_id', user.id).order('created_at', { ascending: false })
            .then(({ data }) => {
                setUserLocks(data || []);
                // Si aucun lock n'est sélectionné via l'URL, on prend le premier par défaut
                if (!selectedLockId && data && data.length > 0) {
  ...
  }, [user]); // On retire searchParams des dépendances pour éviter le reset
  ...
  const handleBoostPurchase = async () => {
    ...
    // On passe TOUTES les infos au checkout
    ...
    <div className="min-h-screen bg-slate-950 text-white font-sans">
      <div className="bg-slate-900/50 backdrop-blur border-b border-slate-800 px-4 py-4 sticky top-0 z-30">
        <div className="container mx-auto max-w-6xl flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => router.back()} className="text-slate-400 hover:text-white"><ArrowLeft className="h-5 w-5" /></Button>
          <h1 className="font-bold text-lg flex items-center gap-2"><Rocket className="text-purple-500 h-5 w-5"/> Boost Center</h1>
        </div>
      </div>
                   ...
                   return (
                     <div key={pkg.id} onClick={() => setSelectedPackage(pkg.id)} className={`relative flex items-center gap-4 p-5 rounded-xl border-2 cursor-pointer transition-all ${isSelected ? `${pkg.border} bg-slate-800/80 shadow-lg scale-[1.02]` : 'border-slate-800 bg-slate-800/30 hover:border-slate-600'}`}>
                       ...
                       <div className="flex-1">
                         <div className="flex items-center gap-2 mb-1"><h4 className="font-bold text-lg">{pkg.title}</h4><Badge variant="outline" className="border-slate-600 text-slate-400 text-[10px]">{pkg.days} Days</Badge></div>
            ...
            <div>
               ...
               {!user ? (
                 ...
                 <div className="grid grid-cols-3 sm:grid-cols-4 gap-3 max-h-48 overflow-y-auto pr-2">
                   {userLocks.map(l => (
                     <div key={l.id} onClick={() => setSelectedLockId(l.id)} className={`relative p-2 rounded-xl border-2 cursor-pointer transition-all ${selectedLockId === l.id ? 'border-emerald-500 bg-emerald-500/10' : 'border-slate-800 bg-slate-800/50'}`}>
...
export default function BoostPage() { return <Suspense fallback={<div className="h-screen flex items-center justify-center bg-slate-950"><Loader2 className="animate-spin text-white"/></div>}><BoostPageContent /></Suspense>; }
