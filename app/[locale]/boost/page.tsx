...
import { Sparkles, Crown, Trophy, Clock, CheckCircle, Loader2, ArrowLeft, Zap, Lock } from 'lucide-react';
...
function BoostPageContent() {
  ...
  const handleBoostPurchase = async () => {
    if (!user) {
        router.push('/purchase'); // Redirection Login
        return;
    }
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="container mx-auto px-4 py-10 max-w-6xl grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            {/* 2. Select Lock (Logique adaptée : Si pas connecté, montre un bouton Login) */}
            <div>
                ...
                {!user ? (
                    <div className="p-8 bg-white border-2 border-dashed border-slate-200 rounded-xl text-center">
                        ...
                        <p className="text-slate-500 mb-4">You must be logged in to see your locks.</p>
                        <Button onClick={() => router.push('/purchase')}>Login to Continue</Button>
                    </div>
                ) : userLocks.length === 0 ? (
          ...
          <div>
            <Card className="sticky top-24 shadow-xl border-amber-100">
              <CardContent className="p-6 space-y-4">
                ...
                <Button onClick={handleBoostPurchase} className="w-full bg-amber-600 hover:bg-amber-700 h-12 text-lg">Pay & Boost 🚀</Button>
              </CardContent>
            </Card>
          </div>
      </div>
