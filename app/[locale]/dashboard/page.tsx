...
function DashboardContent() {
  ...
  // Infos Banque (International)
  const [bankZone, setBankZone] = useState('EU');
  const [field1, setField1] = useState('');
  const [field2, setField2] = useState('');
  
  // Gestion des prix de revente
  ...
  const handleSellToggle = async (lockId: number, currentStatus: string) => {
    ...
    if (currentStatus === 'For_Sale') {
      ...
      if (!error) { 
        toast.success(`Listed for $${priceValue}!`); 
        fetchData(); 
      } else {
                    ...
                    <div className={`flex flex-col gap-3 pt-4 border-t border-slate-100 mt-2 ${lock.status === 'For_Sale' ? 'bg-green-50/50 -mx-6 px-6 py-4' : ''}`}>
                       ...
                       {lock.status === 'For_Sale' ? (
                         <div className="flex items-center justify-between">
                           ...
                           >
                             <XCircle className="w-3 h-3 mr-2"/> Cancel Sale
                           </Button>
                         </div>
                       ) : (
                         <div className="space-y-3">
                           <div className="flex justify-between items-end">
                             <div className="text-sm text-slate-600">
                               ...
                               <p className="text-xs text-slate-400 max-w-[250px] leading-tight">
                                 20% transfer fees applied upon sale.<br/>
                                 You will receive: <strong className="text-green-600">${calculateNetEarnings(priceInputs[lock.id])}</strong>
                               </p>
                             </div>
                             <div className="flex items-center gap-2">
                               <div className="relative">
                                 ...
                                 <Input 
                                   type="number" 
                                   placeholder="Price" 
                                   className="h-9 w-24 pl-6 bg-white" 
                               ...
                               <Button 
                                 ...
                                 onClick={() => handleSellToggle(lock.id, lock.status)}
                               >
                                 <Tag className="w-3 h-3 mr-2"/> Sell
                               </Button>
                             </div>
                           </div>
                         </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <select 
                      className="flex h-10 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm focus:ring-2 focus:ring-[#e11d48]"
                      value={bankZone}
                      onChange={(e) => setBankZone(e.target.value)}
                    >
                      <option value="EU">🇪🇺 Europe (IBAN/SEPA)</option>
                  ...
                  {bankZone === 'EU' && (
                    <>
                      ...
                      <div className="space-y-2"><Label>BIC / SWIFT</Label><Input value={field2} onChange={(e) => setField2(e.target.value)} placeholder="ABCD..." /></div>
                    </>
                  )}
                  {bankZone === 'US' && (
                    <>
                      <div className="space-y-2"><Label>Routing Number</Label><Input value={field2} onChange={(e) => setField2(e.target.value)} placeholder="9 digits" /></div>
                      <div className="space-y-2"><Label>Account Number</Label><Input value={field1} onChange={(e) => setField1(e.target.value)} /></div>
                    </>
                  )}
                  {(bankZone !== 'EU' && bankZone !== 'US') && (
                    <>
                      <div className="space-y-2"><Label>Account Number</Label><Input value={field1} onChange={(e) => setField1(e.target.value)} /></div>
