...
import { 
  ...
  PartyPopper, CreditCard, Wallet, LogOut, DollarSign, Zap, Plus, ArrowUpRight, Loader2, Home, Store 
} from 'lucide-react';
...
function DashboardContent() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // 🛑 CORRECTION CRITIQUE ICI : On cache les 'Pending' (impayés)
      const { data: locksData } = await supabase
        .from('locks')
        .select('*')
        .eq('owner_id', user.id)
        .neq('status', 'Pending') // <--- IL FALLAIT AJOUTER ÇA
            ...
            <div className="flex items-center gap-4 w-full md:w-auto justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-400">Welcome, {profile?.full_name || user?.email}</p>
                    </div>
                </div>
      ...
      <main className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-8">
            <div onClick={() => router.push('/sell')} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 cursor-pointer hover:shadow-md hover:border-emerald-300 transition-all group">
                <div className="flex justify-between items-start mb-4"><div className="bg-emerald-100 p-3 rounded-xl text-emerald-600 group-hover:scale-110 transition-transform"><DollarSign size={24}/></div><ArrowUpRight className="text-slate-300 group-hover:text-emerald-500"/></div>
                <h3 className="font-bold text-lg text-slate-900">Sell Asset</h3><p className="text-slate-500 text-sm">List on marketplace</p>
            </div>
            <div onClick={() => router.push('/boost')} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 cursor-pointer hover:shadow-md hover:border-amber-300 transition-all group">
                <div className="flex justify-between items-start mb-4"><div className="bg-amber-100 p-3 rounded-xl text-amber-600 group-hover:scale-110 transition-transform"><Zap size={24}/></div><ArrowUpRight className="text-slate-300 group-hover:text-amber-500"/></div>
                <h3 className="font-bold text-lg text-slate-900">Boost Visibility</h3><p className="text-sm text-slate-500">Sell 5x faster</p>
            </div>
            <div onClick={() => router.push('/purchase')} className="bg-gradient-to-br from-slate-900 to-slate-800 p-6 rounded-2xl shadow-lg cursor-pointer hover:scale-[1.02] transition-all group text-white">
                <div className="flex justify-between items-start mb-4"><div className="bg-white/20 p-3 rounded-xl text-white group-hover:rotate-90 transition-transform"><Plus size={24}/></div><div className="bg-emerald-500 text-[10px] font-bold px-2 py-1 rounded">NEW</div></div>
                <h3 className="font-bold text-lg">Buy New Asset</h3><p className="text-slate-400 text-sm">Add to your collection</p>
            </div>
        </div>
        ...
        <Tabs defaultValue="assets" className="space-y-6">
                      ...
                      <div className="flex gap-2 mt-3">
                         {lock.status === 'For_Sale' ? (
                           <div className="flex-1 flex items-center justify-between bg-green-50 px-3 py-1.5 rounded-lg border border-green-100">
                              ...
                              <button onClick={() => handleSellToggle(lock.id, lock.status)} className="text-red-500 hover:text-red-700 text-xs font-bold bg-white px-2 py-0.5 rounded border border-red-100">Cancel</button>
                           </div>
                         ) : (
                           <>
                             <div className="relative w-24"><span className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-400 text-xs">$</span><Input className="h-8 pl-5 text-xs pr-1" placeholder="Price" value={priceInputs[lock.id] || ''} onChange={(e) => handlePriceChange(lock.id, e.target.value)} /></div>
                             <Button onClick={() => handleSellToggle(lock.id, lock.status)} size="sm" className="h-8 text-xs bg-emerald-600 hover:bg-emerald-700 text-white">Sell</Button>
                             <Button onClick={() => router.push(`/boost?lock_id=${lock.id}`)} size="sm" variant="outline" className="h-8 text-xs border-amber-300 text-amber-600 hover:bg-amber-50"><Zap size={12}/></Button>
                           </>
                         )}
          ...
          <TabsContent value="bank">
            <div className="grid md:grid-cols-2 gap-8">
              <Card><CardHeader><CardTitle>Payout Method</CardTitle></CardHeader><CardContent className="space-y-4"><div className="space-y-2"><Label>Region</Label><select className="flex h-10 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm" value={bankZone} onChange={(e) => setBankZone(e.target.value)}><option value="EU">🇪🇺 Europe</option><option value="US">🇺🇸 USA</option><option value="UK">🇬🇧 UK</option><option value="OTHER">🌍 Other</option></select></div><div className="space-y-2"><Label>IBAN / Account</Label><Input value={field1} onChange={(e) => setField1(e.target.value)} /></div><div className="space-y-2"><Label>BIC / Routing</Label><Input value={field2} onChange={(e) => setField2(e.target.value)} /></div><Button onClick={handleSaveBank} className="w-full bg-slate-900 text-white mt-2"><Lock className="mr-2 h-4 w-4"/> Secure Save</Button></CardContent></Card>
            </div>
          </TabsContent>
        </Tabs>
      </main>
